...
class DockerEnvironment:
    ...
    def write_file(
        ...
        *,
        ...
        """Create or overwrite a file in the running container.
        ...
        python_code = (
            ...
            f"path = pathlib.Path({file_path!r}); "
            "path.parent.mkdir(parents=True, exist_ok=True); "
            f"path.write_text(base64.b64decode({encoded_content!r}).decode('utf-8'), encoding='utf-8')"
        )
        command = f"python -c {shlex.quote(python_code)}"
        action = {"tool_name": "write_file", "command": command, "arguments": {"path": file_path}}
        return self.execute(action=action, cwd=cwd, timeout=timeout)
    ...
    def read_file(
        ...
        *,
        ...
        python_code = (
            ...
            f"content = pathlib.Path({file_path!r}).read_bytes(); "
            "sys.stdout.write(base64.b64encode(content).decode('ascii'))"
        )
        command = f"python -c {shlex.quote(python_code)}"
        action = {"tool_name": "read_file", "command": command, "arguments": {"path": file_path}}
        result = self.execute(action=action, cwd=cwd, timeout=timeout)
        if result["returncode"] != 0:
            raise RuntimeError(f"Failed to read file {file_path}")
        return base64.b64decode(result["output"]).decode("utf-8")
